...
class Sprite:
    ...
    def collide(self,other_object):
        if dist(self.pos,other_object.get_position()) <= other_object.get_radius() + self.radius:
            return True
        return False
...
def click(pos):
    global started,lives,count,rock_group,score,my_ship
...
def draw(canvas):
    global time, started,rock_group,lives,score,count
...
def rock_spawner():
    global rock_group,count,my_ship,lives
    if count<=12:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        if dist(my_ship.get_position(),rock_pos)<my_ship.get_radius()+40:
            lives+=1
